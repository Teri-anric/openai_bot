...
async def run_conversation(instruction: InstructionGPT, messages: list[TelegramMessages], available_functions: dict[str, Callable[..., Awaitable]]):
    # send the conversation and available functions to the model
    response = await client.chat.completions.create(
        model=instruction.gpt_model,
        messages=prepare_chat_messages(instruction, messages),
        tools=TOOLS,
